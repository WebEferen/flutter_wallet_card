...
public class FlutterWalletCardPlugin: NSObject, FlutterPlugin {
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    let channel = FlutterMethodChannel(
      name: "flutter_wallet_card", binaryMessenger: registrar.messenger())
  ...
  private func addWalletCard(call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard let arguments = call.arguments as? [String: Any],
      ...
      result(
        FlutterError(
          code: "INVALID_ARGUMENTS", message: "Missing or invalid file path", details: nil))
      return
    }
    ...
    guard FileManager.default.fileExists(atPath: filePath) else {
      result(
        FlutterError(
          code: "FILE_NOT_FOUND", message: "Pass file not found at path: \(filePath)", details: nil)
      )
      return
    }
    ...
    guard let passData = NSData(contentsOfFile: filePath) else {
      result(
        FlutterError(code: "FILE_READ_ERROR", message: "Unable to read pass file", details: nil))
      return
    }
    ...
    do {
      ...
      if passLibrary.containsPass(pass) {
        result(
          FlutterError(
            code: "PASS_ALREADY_EXISTS", message: "Pass is already in wallet", details: nil))
        return
      }
      ...
      guard let addPassVC = PKAddPassesViewController(pass: pass) else {
        result(
          FlutterError(
            code: "CANNOT_ADD_PASS", message: "Cannot add this pass to wallet", details: nil))
        return
      }
      ...
      result(
        FlutterError(
          code: "INVALID_PASS", message: "Invalid pass file: \(error.localizedDescription)",
          details: nil))
    }
  }
  ...
  private func addWalletCardFromUrl(call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard let arguments = call.arguments as? [String: Any],
      ...
      result(
        FlutterError(code: "INVALID_ARGUMENTS", message: "Missing or invalid URL", details: nil))
      return
    }
      ...
      DispatchQueue.main.async {
        if let error = error {
          result(
            FlutterError(
              code: "DOWNLOAD_ERROR",
              message: "Failed to download pass: \(error.localizedDescription)", details: nil))
          return
        }
        ...
        do {
          ...
          result(
            FlutterError(
              code: "INVALID_PASS", message: "Invalid pass data: \(error.localizedDescription)",
              details: nil))
        }
      }
    }
  ...
  private func addMultipleWalletCards(call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard let arguments = call.arguments as? [String: Any],
      ...
      result(
        FlutterError(
          code: "INVALID_ARGUMENTS", message: "Missing or invalid file paths", details: nil))
      return
    }
    ...
    for filePath in filePaths {
      guard FileManager.default.fileExists(atPath: filePath) else {
        result(
          FlutterError(
            code: "FILE_NOT_FOUND", message: "Pass file not found at path: \(filePath)",
            details: nil))
        return
      }
      ...
      guard let passData = NSData(contentsOfFile: filePath) else {
        result(
          FlutterError(
            code: "FILE_READ_ERROR", message: "Unable to read pass file at: \(filePath)",
            details: nil))
        return
      }
      ...
      do {
        ...
        result(
          FlutterError(
            code: "INVALID_PASS",
            message: "Invalid pass file at \(filePath): \(error.localizedDescription)", details: nil
          ))
        return
      }
    }
    ...
    guard !passes.isEmpty else {
      result(
        FlutterError(
          code: "NO_NEW_PASSES", message: "All passes are already in wallet", details: nil))
      return
    }
    ...
    guard let addPassVC = PKAddPassesViewController(passes: passes) else {
      result(
        FlutterError(
          code: "CANNOT_ADD_PASSES", message: "Cannot add these passes to wallet", details: nil))
      return
    }
  ...
  private func isWalletCardAdded(call: FlutterMethodCall, result: @escaping FlutterResult) {
    guard let arguments = call.arguments as? [String: Any],
      ...
      result(
        FlutterError(
          code: "INVALID_ARGUMENTS", message: "Missing or invalid serial number", details: nil))
      return
    }
  ...
  private func viewWalletCardInWallet(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    guard let pass = passLibrary.passes().first(where: { $0.serialNumber == serialNumber }) else {
      result(
        FlutterError(
          code: "PASS_NOT_FOUND",
          message: "Pass with serial number \(serialNumber) not found in wallet", details: nil))
      return
    }

    guard let passURL = pass.passURL else {
      result(
        FlutterError(code: "NO_PASS_URL", message: "Pass does not have a valid URL", details: nil))
      return
    }
  ...
  public func addPassesViewControllerDidFinish(_ controller: PKAddPassesViewController) {
    controller.dismiss(animated: true) { [weak self] in
      guard let self = self,
        ...
        let addPassesFlutterResult = self.addPassesFlutterResult
      else {
        return
      }
