...
class AppleWalletGenerator extends CardGenerator {
  ...
  Future<File> generateCard(WalletCard card) async {
    ...
    try {
      ...
      final tempDir = await fileManager.createTempDirectory('apple_pass_${card.id}');
      ...
      // Create .pkpass file
      final outputFile = await fileManager.createOutputFile('${card.id}$fileExtension');
  ...
  Future<File> generateFile(WalletCard card, File outputFile) async {
    ...
    try {
      // Create temporary directory for pass contents
      final tempDir = await Directory.systemTemp.createTemp('apple_pass_${card.id}');
  ...
  void validateCard(WalletCard card) {
    ...
    if (card.metadata.organizationName.isEmpty) {
      throw ArgumentError('Organization name is required for Apple Wallet passes');
    }
    ...
    if (!platformData.containsKey('passTypeIdentifier')) {
      throw ArgumentError('passTypeIdentifier is required for Apple Wallet passes');
    }
  ...
  Map<String, dynamic> _generatePassJson(WalletCard card) {
    ...
    if (card.metadata.locations != null && card.metadata.locations!.isNotEmpty) {
      json['locations'] = card.metadata.locations!.map((loc) => {
  ...
  Future<void> _createPkpassArchive(Directory sourceDir, File outputFile) async {
...
class GoogleWalletGenerator extends CardGenerator {
  ...
  Map<String, dynamic> _generateGoogleWalletJson(WalletCard card) {
    ...
    switch (card.type) {
      case WalletCardType.loyalty:
        json['loyaltyPoints'] = {
          'label': 'Points',
          'balance': {
            'string': card.platformData['points']?.toString() ?? '0'
          }
        };
        break;
    ...
    if (card.metadata.locations != null && card.metadata.locations!.isNotEmpty) {
      json['locations'] = card.metadata.locations!.map((location) => {
