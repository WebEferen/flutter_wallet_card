...
class FileManager {
  ...
  Future<Directory> createTempDirectory(String name) async {
    ...
    final cardTempDir = Directory(path.join(tempDir.path, _defaultDirectoryName, name));
  ...
  Future<Directory> extractArchive(File archiveFile, {String? targetDirName}) async {
    if (!archiveFile.existsSync()) {
      throw WalletException('Archive file does not exist: ${archiveFile.path}');
    }
    
    final targetName = targetDirName ?? path.basenameWithoutExtension(archiveFile.path);
  ...
  Future<File> createArchive(Directory sourceDir, File outputFile) async {
    if (!sourceDir.existsSync()) {
      throw WalletException('Source directory does not exist: ${sourceDir.path}');
    }
  ...
  Future<File> downloadFile(String url, {String? filename}) async {
    try {
      ...
      if (response.statusCode != 200) {
        throw WalletException('Failed to download file: HTTP ${response.statusCode}');
      }
  ...
  Future<void> cleanup({Duration? olderThan}) async {
    ...
    try {
      ...
      final walletTempDir = Directory(path.join(tempDir.path, _defaultDirectoryName));
  ...
  Future<void> cleanupOldFiles(Directory directory, {required Duration maxAge}) async {
